...
export default function DeliveryOrderTable(props: DeliveryOrderTableProps) {
  ...
  const onDONumberClick = (code: number) => {
    ...
    const deliveryOrderDetailRoute = `${ROUTES.DELIVERY_ORDERS}/${code}`;
          ...
          <TableBody>
            {tableData?.map((column) => (
              <TableRow key={column.doId}>
                ...
                <TableCell>
                  {new Date(column.collectionDate)
                ...
                {/* <TableCell> {column?.salesDate ? moment(column.salesDate).format("DD-MMM-YYYY") : "-"}</TableCell>   */}
                <TableCell>
                  {column.salesDate
                ...
                <TableCell>{column.salesCode}</TableCell>
