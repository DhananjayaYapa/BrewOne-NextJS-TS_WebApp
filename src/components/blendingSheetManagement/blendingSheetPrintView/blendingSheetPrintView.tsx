...
import { GetPrintBlendSheetDetail } from "@/interfaces/blendSheet";
import {
  Box,
  Divider,
  Grid,
...
export interface BlendingSheetPrintProps {
  data: GetPrintBlendSheetDetail | null;
  forStore: boolean;
}
export default function BlendingSheetPrintView(props: BlendingSheetPrintProps) {
  const { data, forStore } = props;
  console.log(data, forStore, "print view data");
  ...
  const totalBags = data?.lots.reduce(
    (accumulator, currentVal) => accumulator + currentVal.noOfBags,
    0
  );
  ...
  // Calculate average weight per kg with exact precision
  ...
  const totalBlendGainSieveValue =
    totalBlendBalanceQty + totalGainQty + totalSieveQty;

  const sumOfQty = totalBlendGainSieveValue + totalWeight;
  ...
  return (
    <Grid container>
      ...
      <Grid item lg={4} md={4} sm={4}>
        ...
        <Typography>{moment().format("YYYY-MM-DD hh.mm.ss A")}</Typography>
      </Grid>
          ...
          <TableBody>
            {data?.lots?.map((lot, index) => (
              ...
              >
                ...
                <TableCell align="center">{lot.boxNo}</TableCell>
          ...
          >
            <Table>
              ...
              >
                ...
                {data?.otherBlendItemList?.map((otherBlendItem, index) => {
                  return (
                    <TableRow key={index}>
                      <TableCell>{"-"}</TableCell>
                      <TableCell>{otherBlendItem.batchId}</TableCell>
                      <TableCell align="right">{"-"}</TableCell>
                      <TableCell align="right">{"-"}</TableCell>
                      <TableCell align="right">
                        {otherBlendItem?.quantity
                      ...
                      <TableCell align="center">{"-"}</TableCell>
                      <TableCell align="right">
                        {safeNumber(otherBlendItem.quantity).toFixed(2)}
                      </TableCell>
                      {!forStore && (
                        <TableCell align="right">
                          {otherBlendItem?.price
                            ? otherBlendItem?.price.toFixed(2)
                            : "0.00"}
                        </TableCell>
                      )}
                      {!forStore && (
                        <TableCell align="right">
                          {(() => {
                            ...
                            const blendPrice = safeNumber(otherBlendItem.price);
                            const totalQuantity = sieveQty;
                            const value = totalQuantity * blendPrice;
                            return value
                              .toFixed(2)
                              .replace(/\B(?=(\d{3})+(?!\d))/g, " ");
                ...
                <TableRow>
                  ...
                  >
                    {totalBlendBalanceQty ? totalBlendBalanceQty : "0.00"}
                  </TableCell>
                  <TableCell
                    align="right"
                    sx={{
                  ...
                  >
                    {totalGainQty ? totalGainQty : "0.00"}
                  </TableCell>
                  <TableCell
                    align="right"
                    sx={{
                  ...
                  >
                    {totalSieveQty ? totalSieveQty.toFixed(2) : "0.00"}
                  </TableCell>
                  <TableCell
                    sx={{
                      borderTop: "2px #000000 solid",
                  ...
                  >
                    {totalBlendGainSieveValue
                      ? totalBlendGainSieveValue?.toFixed(2)
                      : "0.00"}
                  </TableCell>
                  {!forStore && (
                    <TableCell
